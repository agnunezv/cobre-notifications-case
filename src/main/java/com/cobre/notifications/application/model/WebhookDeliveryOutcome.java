...
public record WebhookDeliveryOutcome(
    ...
    @AssertTrue(message = "a successful delivery requires a 2xx status and no failure information")
    public boolean isSuccessConsistent() {
        return result == null
    ...
    @AssertTrue(message = "a failed delivery requires consistent failure information")
    public boolean isFailureConsistent() {
        ...
        boolean transportFailureIsConsistent = failureCategory == NotificationDeliveryFailureCategory.HTTP_RESPONSE
                || httpStatus == null;

        return failureCategory != null
                && hasDescription
                && httpFailureIsConsistent
                && transportFailureIsConsistent;
    }
}
