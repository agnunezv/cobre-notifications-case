...
public record NotificationDeliveryAttemptCompletion(
    ...
    @AssertTrue(message = "nextStatus must match the delivery outcome") public boolean isOutcomeConsistent() {
        ...
        return switch (outcome.result()) {
            ...
            case RETRYABLE_FAILURE ->
                nextStatus == DeliveryStatus.RETRY_SCHEDULED || nextStatus == DeliveryStatus.FAILED;
        };
    }

    @AssertTrue(message = "nextAttemptAt is required only for a future scheduled retry") public boolean isRetryScheduleConsistent() {
