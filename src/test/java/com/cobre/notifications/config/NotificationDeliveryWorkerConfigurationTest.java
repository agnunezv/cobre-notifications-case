...
class NotificationDeliveryWorkerConfigurationTest {
    ...
    void keepsScheduledDeliveryDisabledUnlessExplicitlyEnabled() {
        contextRunner.withPropertyValues(workerProperties(false)).run(context -> {
    ...
    void createsTheScheduledWorkerAndBindsHumanReadableDurationsWhenEnabled() {
        contextRunner.withPropertyValues(workerProperties(true)).run(context -> {
            assertThat(context).hasNotFailed();
            ...
            NotificationDeliveryWorkerProperties properties =
                    context.getBean(NotificationDeliveryWorkerProperties.class);
