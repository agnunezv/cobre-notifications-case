...
class MicrometerNotificationDeliveryMetricsTest {
    ...
    void recordsAttemptResultsDurationsAndBoundedFailureTags() {
        ...
        MicrometerNotificationDeliveryMetrics metrics = metrics(registry, false);
        ...
        metrics.recordAttempt(
                delivery(), new WebhookDeliveryOutcome(DeliveryAttemptResult.SUCCESS, 202, null, null, 40));
    ...
    void recordsWorkerOutcomesAndSuccessfulOrFailedBatchDurations() {
        SimpleMeterRegistry registry = new SimpleMeterRegistry();
        MicrometerNotificationDeliveryMetrics metrics = metrics(registry, true);

        metrics.recordBatch(new NotificationDeliveryBatchResult(2, 5, 1, 1, 1, 2), Duration.ofMillis(250));
    ...
    private MicrometerNotificationDeliveryMetrics metrics(SimpleMeterRegistry registry, boolean workerEnabled) {
        return new MicrometerNotificationDeliveryMetrics(
                ...
                new com.cobre.notifications.config.NotificationDeliveryWorkerProperties(
                        workerEnabled, "test-worker", 10, Duration.ofSeconds(1), Duration.ZERO, Duration.ofMinutes(2)));
    }
